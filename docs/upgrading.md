...
| --- | --- | --- |
| `BOWERBIRD_LIVE_INSTANCE` | unset / false | Treats the repo as an active personal instance. `pull-bookmarks`, `account-dump`, `compile-wiki`, and `recap` run only when this is `true`; setup sets it after required ingest secrets exist. The canonical source repo is additionally blocked by repository identity. |
