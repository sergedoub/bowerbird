...
def test_writes_posts_and_replies(tmp_path):
    accounts = [Account(handle="account_one", topic="ai-updates")]
    client = FakeTimeline({"account_one": [_post("100"), _post("200", reply_to="100")]})
    writer = RawWriter(tmp_path)

    s = run_dump(accounts, client, writer)
    assert s["seen"] == 2 and s["written"] == 2 and s["skipped"] == 0
    assert (tmp_path / "account_one" / "2026-05-01__100.md").exists()
    assert (tmp_path / "account_one" / "2026-05-01__200.md").exists()


def test_rerun_is_idempotent_forward_only(tmp_path):
    accounts = [Account(handle="account_one", topic="ai-updates")]
    client = FakeTimeline({"account_one": [_post("100")]})
    writer = RawWriter(tmp_path)
...
def test_windowed_rerun_stops_after_first_all_seen_page(tmp_path):
    ...
    writer = RawWriter(tmp_path)
    run_dump(accounts, FakeTimeline({"account_one": pages}), writer, start_time="2026-05-21T00:00:00Z")

    client = FakeTimeline({"account_one": pages})
...
def test_full_run_never_stops_early(tmp_path):
    ...
    run_dump(accounts, FakeTimeline({"account_one": [[_post("300")]]}), writer,
             start_time="2026-05-21T00:00:00Z")

    client = FakeTimeline({"account_one": [[_post("300")], [_post("100")]]})
...
def test_page_with_any_new_post_keeps_walking(tmp_path):
    ...
    client = FakeTimeline({"account_one": [[_post("400"), _post("300")], [_post("200")]]})
    s = run_dump(accounts, client, writer, start_time="2026-05-21T00:00:00Z")
    assert client.pages_served == 2
...
def test_max_posts_caps_processing_and_requested_page_size(tmp_path):
    accounts = [Account(handle="account_one", topic="ai-updates")]
    client = FakeTimeline({"account_one": [[_post(str(i)) for i in range(100, 110)]]})
...
def test_max_posts_stops_paging_across_pages(tmp_path):
    accounts = [Account(handle="account_one", topic="ai-updates")]
    client = FakeTimeline({"account_one": [[_post("300"), _post("200")], [_post("100")]]})
...
def test_build_raw_doc_marks_replies_and_keeps_parent():
    doc = build_raw_doc(_post("200", reply_to="100"), "account_one")
    assert doc.frontmatter["account"] == "@account_one"
    assert doc.frontmatter["is_reply"] is True
    assert doc.frontmatter["in_reply_to"] == "100"


def test_build_raw_doc_marks_original_post():
    doc = build_raw_doc(_post("100"), "account_one")
...
def test_build_raw_doc_prefers_note_tweet_and_captures_article():
    ...
    doc = build_raw_doc(t, "account_one")
