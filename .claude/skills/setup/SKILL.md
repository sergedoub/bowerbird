---
name: setup
description: Set up this Bowerbird instance end to end — credentials, X app, OAuth, folder mapping, GitHub Actions secrets, first pull, web app, Slack recap. Use when the user wants to set up, configure, install, or onboard this freshly cloned repository.
---
...
- When the user must choose between options (which folders to watch,
  which accounts to follow, deploy now vs later, quiet-day recap
  preference), prefer a structured user-choice tool such as AskUserQuestion
  or request_user_input when it is actually callable in the current mode. If
  the tool is unavailable or rejected, immediately ask the same question as a
...
- Browser control should be extension-backed. In Claude Code, opt in via
  `claude --chrome` or `/chrome`; in Codex, use the Chrome extension bridge
  and verify Chrome shows it is being controlled/debugged by Codex. If attach
  fails or macOS asks for System Events/Desktop UI control, pause and ask the
  user to reconnect/restart the extension instead of silently falling back on
  credential pages. With extension-backed control, offer to drive the X
  developer portal and GitHub PAT pages with the user's logged-in session —
  they handle login and payment screens.
- If ordinary browser UI blocks progress, try standard browser escape hatches
  ...
  user-owned.
- Clipboard rule (only when you have extension-backed browser control): for EVERY credential
...
4. **Remaining credentials** — stage into bin/.env, then push:
   ...
   b. Model provider key: default to the active agent's provider. In Claude
      Code this can be Anthropic: `bowerbird models --provider anthropic
      --write`, open https://console.anthropic.com/settings/keys, create a
      Bowerbird API key, and stage it as ANTHROPIC_API_KEY per the clipboard
      rule. If the user selects Codex/OpenAI or Gemini instead, use
      `bowerbird models --provider openai --write` plus OPENAI_API_KEY or
      `bowerbird models --provider gemini --write` plus GEMINI_API_KEY.
      Hosted CI cannot use a local app subscription directly; it needs the
      API key secret.
...
5. **Localhost control panel** — onboarding moves to the browser. Build
   ...
   - choose which folders Bowerbird watches on the Folders page;
   - add accounts to follow on the Config page.
...
8. **Deploy (optional)** — Vercel: import the repo, Root Directory =
   `web/`, same env vars (APP_URL becomes the vercel.app URL; add
   `<app>.vercel.app/api/auth/callback` as a third redirect URI on the X
   app); test Sign in with X on the deployed URL. For hosted recap delivery,
   set SLACK_WEBHOOK_URL plus optional selected provider API key and
   CRON_SECRET; trigger /api/cron/recap once and confirm delivery.

9. **Wrap-up** — status table: what works (tests, lint, each workflow,
   web app local/deployed, recap, Slack delivery if configured), what the user
